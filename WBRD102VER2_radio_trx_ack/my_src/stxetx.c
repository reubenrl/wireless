...
static void (*stxetxDataOut)(uint8_t data);
//static void (*stxetxUartDataOut)(uint8_t nUart, uint8_t data);
//#define stxetxDataOut(d)		stxetxUartDataOut(COM_STXETX_DATA, (d))
// received packet data buffer
uint8_t stxetxRxPacket[STXETX_MAXRXPACKETSIZE];

// functions


// Initialize STX/ETX packet protocol library
void stxetxInit(void (*dataout_func)(uint8_t data))
{
	stxetxDataOut = dataout_func;
}

// Send/Create STX/ETX packet
void stxetxSend(uint8_t status, uint8_t type, uint8_t datalength, uint8_t* dataptr)
{
	uint8_t checksum = 0;
	unsigned short i;
...
uint8_t stxetxProcess(cBuffer* rxBuffer)
{
	uint8_t foundpacket = false;
	unsigned short i;
...
uint8_t stxetxGetRxPacketStatus(void)
{
...
uint8_t stxetxGetRxPacketType(void)
{
...
uint8_t stxetxGetRxPacketDatalength(void)
{
...
uint8_t* stxetxGetRxPacketData(void)
{
